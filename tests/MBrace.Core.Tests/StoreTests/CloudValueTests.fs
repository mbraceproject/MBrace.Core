...
type ``CloudValue Tests`` (parallelismFactor : int) as self =
    ...
    member __.``CloudArray count`` () =
        ...
        cloud {
            ...
            let! job = Cloud.StartJob(cloud { value'.Length |> shouldEqual size })
            return! job.AwaitResult()
        } |> runOnCloud
    ...
    member __.``Dispose CloudValue`` () =
        ...
        fun () ->
            cloud {
                ...
                do! Cloud.Sleep 2000
                let! job = Cloud.StartJob(cloud { return! cv.GetValueAsync() })
                return! job.AwaitResult()
            } |> runOnCloud
