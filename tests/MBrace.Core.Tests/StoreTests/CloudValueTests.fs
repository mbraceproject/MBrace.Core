...
type ``CloudValue Tests`` (parallelismFactor : int) as self =
    ...
    member __.``Remote value memory cached`` () =
        if __.IsSupportedLevel StorageLevel.MemoryAndDisk then
            ...
            cloud {
                ...
                let! job = Cloud.StartCloudProcess(jobF())
    ...
    member __.``CloudArray count`` () =
        ...
        cloud {
            ...
            let! job = Cloud.StartCloudProcess(cloud { value'.Length |> shouldEqual size })
            return! job.AwaitResult()
        } |> runOnCloud
    ...
    member __.``Dispose CloudValue`` () =
        ...
        fun () ->
            cloud {
                ...
                do! Cloud.Sleep 2000
                let! job = Cloud.StartCloudProcess(cloud { return! cv.GetValueAsync() })
                return! job.AwaitResult()
            } |> runOnCloud
    ...
    member __.``Get CloudValue by Id`` () =
        ...
        cloud {
            ...
            let! job = Cloud.StartCloudProcess(cloud {
