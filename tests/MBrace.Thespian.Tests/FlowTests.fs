...
type ``MBrace Thespian Flow Tests`` () =
    ...
    override __.RunOnCloud(expr : Cloud<'T>) : 'T = session.Runtime.Run(expr, faultPolicy = FaultPolicy.NoRetry)
    override __.RunOnCurrentMachine(expr : Cloud<'T>) : 'T = session.Runtime.RunOnCurrentMachine(expr)
