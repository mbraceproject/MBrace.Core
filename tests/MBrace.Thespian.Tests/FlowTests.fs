...
type ``MBrace Thespian Flow Tests`` () =
    ...
    override __.RunInCloud(expr : Cloud<'T>) : 'T = session.Runtime.Run(expr, faultPolicy = FaultPolicy.NoRetry)
    override __.RunOnClient(expr : Cloud<'T>) : 'T = session.Runtime.RunOnClient(expr)
