...
namespace MBrace.Flow.CSharp.Tests
{
    ...
    abstract public class CloudFlowTests
    {
        ...
        abstract public T RunOnClient<T>(MBrace.Cloud<T> c);
        ...
        public void ToCloudArray()
        {
            FSharpFunc<int[], bool>.FromConverter(xs =>
            {
                ...
                var y = xs.Select(i => i + 1).ToArray();
                var r = this.RunOnClient(this.Run(x).ToEnumerable());
            ...
            FSharpFunc<string[], bool>.FromConverter(xs =>
            {
                ...
                var x =
                    this.Run(cfiles
                        ...
                        .ToArray());
                var y = cfiles.Select(f => this.RunOnClient(CloudFile.ReadAllText(f, null)));
            ...
            FSharpFunc<string[][], bool>.FromConverter(xs =>
            {
                ...
                var x =
                    this.Run(cfiles
                        ...
                        .ToArray());
                var y = cfiles.Select(f => this.RunOnClient(CloudFile.ReadAllLines(f,null)))
                        .SelectMany(id => id);
