...
type PersistedCloudFlow<'T> internal (partitions : (IWorkerRef * CloudArray<'T>) []) =
    ...
    member __.ElementCount : int64 = partitions |> Array.sumBy (fun (_,p) -> int64 p.Length)
    ...
    interface ITargetedPartitionCollection<'T> with
        ...
        member __.GetCountAsync(): Async<int64> = async { return __.ElementCount }
...
and PersistedCloudFlow private () =
    ...
    static member OfCloudFlow (flow : CloudFlow<'T>, [<O;D(null:obj)>]?storageLevel : StorageLevel, [<O;D(null:obj)>]?partitionThreshold:int64) : Cloud<PersistedCloudFlow<'T>> = cloud {
