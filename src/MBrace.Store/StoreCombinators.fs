...
type Nessos.MBrace.Store.CloudFile with
    ...
    static member Create(serializer : Stream -> Async<unit>, ?path : string) : Cloud<CloudFile> = cloud {
        ...
        let path = match path with Some p -> p | None -> provider.CreateUniqueFileName "TODO : create process-bound container"
        return! Cloud.OfAsync <| provider.CreateFile(path, serializer)
    }
