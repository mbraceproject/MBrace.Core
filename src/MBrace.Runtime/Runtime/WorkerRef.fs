...
type WorkerRef private (runtime : IRuntimeManager, workerId : IWorkerId) =
    ...
    let [<IgnoreDataMember>] mutable logObservable = None
    let [<IgnoreDataMember>] mutable lockObj = new obj()

    let getObservable() =
        match logObservable with
        | Some o -> o
        | None ->
            lock lockObj (fun () ->
                match logObservable with
                | None ->
                    let o = runtime.WorkerManager.GetWorkerLogObservable workerId |> Async.RunSync
                    logObservable <- Some o
                    o
                | Some o -> o)
    ...
    member __.GetSystemLogsAsync() = runtime.WorkerManager.GetWorkerLogs(workerId)
    /// Fetches all system logs generated by worker instance
    member __.GetSystemLogs() = runtime.WorkerManager.GetWorkerLogs(workerId) |> Async.RunSync
    /// Prints all system logs generated by worker instance to stdout
    member __.ShowSystemLogs(?logLevel : LogLevel) = 
        let logLevel = defaultArg logLevel LogLevel.Debug
        __.GetSystemLogs()
        |> Seq.filter (fun e -> e.LogLevel <= logLevel)
        |> Seq.map (fun e -> SystemLogEntry.Format(e, showDate = true))
        |> String.concat Environment.NewLine
        |> Console.WriteLine

    /// Gets a local observable on system logs generated by the worker
    member __.SystemLogs = getObservable()
