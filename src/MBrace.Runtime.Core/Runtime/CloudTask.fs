...
and [<Sealed; DataContract; NoEquality; NoComparison>] CloudTask<'T> internal (source : ICloudTaskCompletionSource, id : IRuntimeId, ?runtime : IRuntimeManager) =
    ...
    override __.GetLogsAsync() = async { 
        let! entries = getRuntime().CloudLogManager.GetAllCloudLogEntriesByTask __.Id
        return entries |> Seq.sortBy (fun e -> e.DateTime) |> Seq.toArray
    }

    override __.GetLogs() = 
        getRuntime().CloudLogManager.GetAllCloudLogEntriesByTask __.Id
        |> Async.RunSync
        |> Seq.sortBy(fun e -> e.DateTime)
        |> Seq.toArray

    override __.ShowLogs () =
        let entries = getRuntime().CloudLogManager.GetAllCloudLogEntriesByTask __.Id |> Async.RunSync
