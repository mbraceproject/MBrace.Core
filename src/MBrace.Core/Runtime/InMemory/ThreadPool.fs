...
module private SchedulerInternals =
    ...
    let scheduleTask res ct sc ec cc (Body f) =
...
type ThreadPool =
    ...
    static member StartChild (computation : Cloud<'T>, ?timeoutMilliseconds) : Cloud<Cloud<'T>> = cloud {
        ...
        let asyncWorkflow = toAsync resources computation
        let! chWorkflow = ofAsync <| Async.StartChild(asyncWorkflow, ?millisecondsTimeout = timeoutMilliseconds)
        return ofAsync chWorkflow
    }