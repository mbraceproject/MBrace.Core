...
[<Sealed; DataContract>]
type DomainLocal<'T> internal (factory : Local<'T>) =
...
type DomainLocal =
    ...
    static member Create(factory : Local<'T>) = new DomainLocal<'T>(factory)

    /// <summary>
    ///     Creates a new DomainLocal entity with supplied factory.
    /// </summary>
    /// <param name="factory">Factory function.</param>
    static member Create(factory : unit -> 'T) = new DomainLocal<'T>(local { return factory () })