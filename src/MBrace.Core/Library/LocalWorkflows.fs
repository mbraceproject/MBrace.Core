...
module Sequential =
    ...
    let map (mapper : 'T -> CloudLocal<'S>) (source : seq<'T>) : CloudLocal<'S []> = local {
    ...
    let filter (predicate : 'T -> CloudLocal<bool>) (source : seq<'T>) : CloudLocal<'T []> = local {
    ...
    /// <param name="source">Input sequence.</param>
    let choose (chooser : 'T -> CloudLocal<'S option>) (source : seq<'T>) : CloudLocal<'S []> = local {
        let results = new ResizeArray<'S> ()
        for t in source do
            let! r = chooser t
    ...
    let fold (folder : 'State -> 'T -> CloudLocal<'State>) (state : 'State) (source : seq<'T>) : CloudLocal<'State> = local {
        let state = ref state
        for t in source do
    ...
    let collect (collector : 'T -> CloudLocal<#seq<'S>>) (source : seq<'T>) : CloudLocal<'S []> = local {
    ...
    /// <param name="collector">Collector function.</param>
    /// <param name="source">Source data.</param>
    let lazyCollect (collector : 'T -> CloudLocal<#seq<'S>>) (source : seq<'T>) : CloudLocal<seq<'S>> = local {
    ...
    /// <param name="predicate">Predicate function.</param>
    /// <param name="source">Input sequence.</param>
    let tryFind (predicate : 'T -> CloudLocal<bool>) (source : seq<'T>) : CloudLocal<'T option> = local {
    ...
    /// <param name="chooser">Choice function.</param>
    /// <param name="source">Input sequence.</param>
    let tryPick (chooser : 'T -> CloudLocal<'S option>) (source : seq<'T>) : CloudLocal<'S option> = local {
        use e = source.GetEnumerator()
        let rec aux () = local {
            if e.MoveNext() then
    ...
    /// <param name="source">Input sequence.</param>
    let iter (body : 'T -> CloudLocal<unit>) (source : seq<'T>) : CloudLocal<unit> = local {
