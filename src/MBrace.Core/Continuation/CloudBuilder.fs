...
type WorkflowBuilder () =
    ...
    member __.For(ts : seq<'T>, body : 'T -> Workflow<'Ctx, unit>) : Workflow<'Ctx, unit> = 
        match ts with
        | :? ('T []) as ts -> forArray body ts
        | :? ('T list) as ts -> forList body ts
        | _ -> forSeq body ts

    member __.While(pred : unit -> bool, body : Workflow<'Ctx, unit>) : Workflow<'Ctx, unit> = whileM pred body
