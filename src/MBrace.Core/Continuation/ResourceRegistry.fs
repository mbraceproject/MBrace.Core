...
type ResourceRegistry private (index : Map<string, obj>) =
    ...
    member __.TryResolve<'TResource> () = index.TryFind typeId<'TResource> |> Option.map unbox<'TResource>

    /// Resolves resource of given type
    member __.Resolve<'TResource> () =
        match index.TryFind typeId<'TResource> with
    ...
    member __.Register<'TResource>(resource : 'TResource) = new ResourceRegistry(Map.add typeId<'TResource> (box resource) index)
